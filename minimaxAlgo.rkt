...
(define (mini-max state cP depth phase cnt)
  (define (func1 bx)
    (cond [(= cP 1) (begin (define nw-state (make-3d-vector 3 3 3 0))
                           ...
                           (3vs nw-state bx 1)
                           (cond [(checkS nw-state bx 1) (let* ((aim2 (all-in-mill nw-state 2))
          ...
          [(= cP 2) (begin (define nw-state (make-3d-vector 3 3 3 0))
                           (vec-copy nw-state state)
                           (3vs nw-state bx 2)
                           (cond [(checkS nw-state bx 2) (let* ((aim1 (all-in-mill nw-state 1))
  ...
  (define (func2 move)
    (cond [(= cP 1) (begin (define nw-state (make-3d-vector 3 3 3 0))
                           ...
                           (cond [(checkS nw-state (cdr move) 1) (let* ((aim2 (all-in-mill nw-state 2))
                                                                        (p2b (filter (λ(x) (and (= (3vf nw-state x) 2)
                                                                               (or aim2 (not (checkS nw-state x 2))))) all-list))
                                                       (vls (map (λ(x) (begin (3vs nw-state x 0)
                                                                              (define temp (mini-max nw-state 2 (- depth 1) phase cnt))
          ...
          [(= cP 2) (begin (define nw-state (make-3d-vector 3 3 3 0))
                           ...
                           (cond [(checkS nw-state (cdr move) 2) (let* ((aim1 (all-in-mill nw-state 1))
                                                                        (p2b (filter (λ(x) (and (= (3vf nw-state x) 1)
                                                                               (or aim1 (not (checkS nw-state x 1))))) all-list))
                                                       (vls (map (λ(x) (begin (3vs nw-state x 0)
                                                                              (define temp (mini-max nw-state 1 (- depth 1) (if (> cnt 9) 1 0) (+ 1 cnt)))
